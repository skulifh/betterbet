""" containing the functions statuses() and clean_status() """
# -*- coding: utf-8 -*-
__author__ = 'Asgeir'
...
def statuses(twitter):
    ...
    for ids in rows:
        ...
        elif (party == "D" and d_count >= desired_d) or \
                (party == "R" and r_count >= desired_r):
            continue
        ...
        try:
            ...
            if error.error_code == 429:
                while True:
                    try:
                        user_timeline = \
                            twitter.get_user_timeline(id=str(ids[0]), count=10)
        ...
        for i in lineslist: #loop through all the lines in the list
            ...
            #loop through the word list to add them to another list
            for j in words:
                if j.find("...") <= 0:
                    wordslist.append(j)
    ...
    #    print "PARTY " + str(IDs[1])
        if guessLanguage(liststr) == "en":
            cur.execute("INSERT INTO final_users_en(users_id, party) "
                        "VALUES(?, ?)", (str(ids[0]), str(ids[1])))
            cur.execute("INSERT INTO statuses(users_id, statuses) "
                        "VALUES(?, ?)",(repr((ids[0])), repr(liststr)))
            if party == "D":
                d_count += 1
            elif party == "R":
                r_count += 1
...
def clean_status(status):
    ...
    status = re.sub(r'(?i)\b((?:https?://|www\d{0,3}[.]|'
                    r'[a-z0-9.\-]+[.][a-z]{2,4}  +   '
                    r'/)(?:[^\s()<>]+|\(([^\s()<>]+|'
                    r'(\([^\s()<>]+\)))*\))+'
                    r'(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|'
                    r'[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))', '', status)
                    #Removes URLs from the status
